...
export default function TradingDashboard() {
  ...
  const addView = useCallback(() => {
    setPanels((prev) => {
      if (prev.length === 0) {
        // Convert single view to multi-view: first panel is current state, add a new one
  ...
  // When selecting a symbol, update the active panel in multi-view, or single view
  ...
  // Grid layout class based on panel count
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <main className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-4">
            ...
            {isMultiView && (
              <span className="text-xs text-muted-foreground font-mono">
                Multi-View &middot; {panels.length} charts
              </span>
            )}
        ...
        {/* Chart Controls (single view mode) */}
        <ChartControls
          chartType={chartType}
        ...
        <div className="flex-1 overflow-auto relative">
          {panels.length > 1 ? (
            ...
            <>
              ...
              <TradingChart
                key={`single-${selectedSymbol}-${chartType}-${indicators.join(",")}-${timeRange}-${isLive ? "live" : "fb"}`}
                data={data}
                chartType={chartType}
                indicators={indicators}
                symbol={selectedSymbol}
